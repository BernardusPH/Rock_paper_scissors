import { advancedGameImgObj,ImgData } from "../../../../game_Img_storage/advancedGameStorage.ts";
...
const ShapeGeneratorAdvanced = ({colorPicked}:ShapeGeneratoradvancedProps) => {
  let generatedShapes = advancedGameImgObj.map((item: ImgData) => (
    <ShapeButton shapeScale={1} colorPicked={colorPicked} {...item} key={item.name} />
  ));
